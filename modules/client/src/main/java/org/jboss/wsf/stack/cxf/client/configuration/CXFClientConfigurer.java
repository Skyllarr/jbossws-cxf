...
public class CXFClientConfigurer extends ConfigHelper
{
   ...
   private void enableElytronClientAuthentication(WildFlyClientConfigProvider elytronProvider, Client cxfClient, String endpointAddress) {
      ...
      setElytronConduitSelector(cxfClient);
      if (ElytronClientConfig.getUsername() != null) {
         if ((ElytronClientConfig.getHttpMechanism() == null && ElytronClientConfig.getWsSecurityType() == null) ||
                 ElytronClientConfig.getHttpMechanism() == ElytronClientConfig.HttpMechanismType.BASIC) {
            useHttpBasic(cxfClient);
         }
         if (ElytronClientConfig.getWsSecurityType() == ElytronClientConfig.WsSecurityType.USERNAME_TOKEN) {
            useUsernameToken(cxfClient);
         }
      }
   }

   private void useHttpBasic(Client cxfClient) {
   ...
   private void useUsernameToken(Client cxfClient) {
      Map<String, Object> requestContext = cxfClient.getRequestContext();
